...
class ModelCatalogCategory extends Model {
	...
	public function getCategories($data = array()) {
		if(!empty($data['filter_store']))
            {
                $join = "LEFT JOIN ".DB_PREFIX."category_to_store cts ON (c1.category_id = cts.category_id)";
                $join_and = " AND cts.store_id = '".(int)$data['filter_store']."'";
            }
            else
            {
                $join = "";
			...
			$sql = "SELECT cp.category_id AS category_id, GROUP_CONCAT(cd1.name ORDER BY cp.level SEPARATOR '&nbsp;&nbsp;&gt;&nbsp;&nbsp;') AS name, c1.parent_id, c1.sort_order FROM " . DB_PREFIX . "category_path cp LEFT JOIN " . DB_PREFIX . "category c1 ON (cp.category_id = c1.category_id) LEFT JOIN " . DB_PREFIX . "category c2 ON (cp.path_id = c2.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd1 ON (cp.path_id = cd1.category_id) LEFT JOIN " . DB_PREFIX . "category_description cd2 ON (cp.category_id = cd2.category_id) ".$join." WHERE cd1.language_id = '" . (int)$this->config->get('config_language_id') . "' AND cd2.language_id = '" . (int)$this->config->get('config_language_id') . "'".$join_and;
	...
	public function getTotalCategories($data) {
		if(!empty($data['filter_store']))
            {
                $join = " LEFT JOIN ".DB_PREFIX."category_to_store cts ON (c.category_id = cts.category_id)";
                $join_and = " WHERE cts.store_id = '".(int)$data['filter_store']."'";
            }
            else
            {
                $join = "";
			...
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category c".$join.$join_and);
