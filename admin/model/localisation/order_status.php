...
class ModelLocalisationOrderStatus extends Model {
	public function getColorCode($order_status_id) {
		$result = $this->db->query("SELECT color_code FROM ".DB_PREFIX."order_status WHERE order_status_id = '".(int)$order_status_id."'");
		
		if($result->num_rows > 0) {
			return $result->row['color_code'];
		}
	...
	public function addOrderStatus($data) {
		foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET order_status_id = '" . (int)$order_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', color_code = '".$this->db->escape($data['color_code'])."'");			
			} else {
	...
	public function editOrderStatus($order_status_id, $data) {
		...
		foreach ($data['order_status'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', color_code = '".$this->db->escape($data['color_code'])."'");
		}
	...
	public function getOrderStatuses($data = array()) {
		if ($data) {
			...
			if (!$order_status_data) {
				$query = $this->db->query("SELECT color_code, order_status_id, name FROM " . DB_PREFIX . "order_status WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name");
