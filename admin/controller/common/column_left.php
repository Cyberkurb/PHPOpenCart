...
class ControllerCommonColumnLeft extends Controller {
	public function index() {
		if (isset($this->request->get['user_token']) && isset($this->session->data['user_token']) && ($this->request->get['user_token'] == $this->session->data['user_token'])) {
			...
			// Store List
			$check_status = $this->config->get('module_store_locator_status');
				if($check_status == 1)
			{
				$store = array(); 
				if ($this->user->hasPermission('access', 'store/store_lists')) {
				$store[] = array(
				'name' => $this->language->get('text_store_list'),
				'href' => $this->url->link('store/store_lists', 'user_token=' . $this->session->data['user_token'], true),
				'children' => array()
				); 
				}
			...
			if ($this->user->hasPermission('access', 'user/activity')) {        
                $user[] = array(
                    'name'     => 'User Activity',
