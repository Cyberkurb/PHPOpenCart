...
class ControllerSaleOrder extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$query->num_rows) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "order_status` ADD `color_code` varchar( 255 ) AFTER `name`");
		}
	}
	...
	public function delete() {
        // Add to activity log
        if($this->request->get['activity_order_id']){
            $this->load->model('user/activity');
            $this->model_user_activity->addActivity('delete_order', 'orderID '.$this->request->get['activity_order_id']);
        }  
		$this->load->language('sale/order');
	...
	protected function getList() {
		...
		$data['payment_countries'] = $this->model_sale_order->getPaymentCountries();
		$data['shipping_methods'] = $this->model_sale_order->getShippingMethods();
		$data['shipping_countries'] = $this->model_sale_order->getShippingCountries();
		...
		if (isset($this->request->get['filter_payment_method'])) {
			$filter_payment_method = urldecode($this->request->get['filter_payment_method']);
			
			$filter_payment_method = explode(",", $filter_payment_method);
		} else {
			$filter_payment_method = null;
		}
		
		if (isset($this->request->get['filter_shipping_method'])) {
			$filter_shipping_method = urldecode($this->request->get['filter_shipping_method']);
			
			$filter_shipping_method = explode(",", $filter_shipping_method);
		} else {
			$filter_shipping_method = null;
		}
		
		if (isset($this->request->get['filter_payment_country'])) {
			$filter_payment_country = urldecode($this->request->get['filter_payment_country']);
			
			$filter_payment_country = explode(",", $filter_payment_country);
		} else {
			$filter_payment_country = null;
		}
		
		if (isset($this->request->get['filter_shipping_country'])) {
			$filter_shipping_country = urldecode($this->request->get['filter_shipping_country']);
			
			$filter_shipping_country = explode(",", $filter_shipping_country);
		} else {
			$filter_shipping_country = null;
		}
		
		if (isset($this->request->get['filter_customer_group_id'])) {
			$filter_customer_group_id = urldecode($this->request->get['filter_customer_group_id']);
			
			$filter_customer_group_id = explode(",", $filter_customer_group_id);
		} else {
		...
		if (isset($this->request->get['filter_order_status'])) {
			$filter_order_status = urldecode($this->request->get['filter_order_status']);
			  //$filter_order_status = explode(",", $filter_order_status);
		} else {
			$filter_order_status = $this->config->get('config_show_order');
            $filter_order_status = $this->config->get('config_order_status_id');
		}
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'gpid'			=> $result['invoice_no'],
				'customer'      => $result['customer'],
				'payment_m'		=> $result['payment_method'],
				'order_status'  => $result['order_status'] ? "<span style='color: ".$result['order_status_color_code'].";'>".$result['order_status']."</span>" : $this->language->get('text_missing'),
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
                ...
                $status_info = $this->model_sale_order->getOrderStatusInfo($this->request->get['order_id'], $product['order_product_id']);
				$data['products'][] = array(
                    ...
                    'tracking'         => $status_info['tracking_number'],
                    'docnum'           => $status_info['documentnum'],
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
                    ...
                    $status_info = $this->model_sale_order->getOrderStatusInfo($order_id, $product['order_product_id']);
