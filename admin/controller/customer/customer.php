...
class ControllerCustomerCustomer extends Controller {
	...
	protected function getList() {
        ...
        $data['sort_phone'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.telephone' . $url, true);
	...
	protected function getForm() {
        ...
        $this->load->model('customer/customer');
        $data['availProducts'] = $this->model_customer_customer->availProducts();
	...
	public function addHistory() {
		...
		if (!$this->user->hasPermission('modify', 'customer/customer')) {
            ...
            $this->model_customer_customer->addHistory($this->request->get['customer_id'], $this->request->post['comment']." -- Note added by: ".$user_info['username']);
    ...
    public function products() {
		...
		$results = $this->model_customer_customer->getProducts($this->request->get['customer_id'], ($page - 1) * 10, 10);

		foreach ($results as $result) {

			$order_id = $this->model_customer_customer->checkCoupon($result['couponcode']);
			...
			$data['products'][] = array(
				...
				'purchasedate'          => date($this->language->get('date_format_short'), strtotime($result['purchasedate'])),
				'purchaselocation'      => $result['purchaselocation'],
                'dateregistered'        => date($this->language->get('date_format_short'), strtotime($result['dateregistered'])),
                'purchaseproduct_id'    => $result['purchaseproduct_id'],
                'coupon_id'             => $result['coupon_id'],
				'couponcode'            => $result['couponcode'],
				'order_id'				=> $order_id,
                'purchaseproduct_name'  => $result['purchaseproduct_name'],
                'user_token'    => $this->session->data['user_token']
			);
		}

		$product_total = $this->model_customer_customer->getTotalProducts($this->request->get['customer_id']);

		$pagination = new Pagination();
		$pagination->total = $product_total;
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('customer/customer/product', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $this->request->get['customer_id'] . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('customer/customer_product', $data));
	}
    ...
    public function addProduct() {
        ...
        if (isset($this->request->post['purchaselocation'])) {
			...
			$user_info = $this->model_user_user->getUser($this->user->getId());
			$addedProduct = $this->model_customer_customer->addProduct($this->request->get['customer_id'], $data);
            if($addedProduct > 1){
                $this->model_customer_customer->addHistory($this->request->get['customer_id'], $user_info['username']. " - manually added registered product - ".$this->request->post['serialnumber']);
                $json['success'] = "Product Registered Added";
            }
            else{
                $json['error'] = "Product is Not Valid";
            }
		}
    ...
    public function orders() {
		...
		$results = $this->model_customer_customer->getOrders($this->request->get['customer_id']);
