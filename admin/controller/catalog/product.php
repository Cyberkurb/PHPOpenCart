...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
        ...
        $this->model_user_activity->addActivity('create_product', 'productID '.$product_id);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_user_activity->addActivity('update_product', 'productID '.$this->request->get['product_id']);
			$this->session->data['success'] = $this->language->get('text_success');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $product_id) {
                ...
                $this->model_user_activity->addActivity('delete_product', 'productID '.$product_id);
			}
	...
	protected function getList() {
	   ...
	   $data['stores'][] = array('store_id' => 0, 'name' => $store_name);
	   ...
	   usort($data['categories'], function ($a, $b) {
		...
		if (isset($this->request->get['filter_category'])) {
			$filter_category = urldecode($this->request->get['filter_category']);
		...
		$filter_data = array(
			...
			'sort'            => $sort,
			'order'           => $order,
			'0'           => ($page - 1) * $this->config->get('config_limit_admin'),
		...
		foreach ($results as $result) {
			if (substr($result['image'],0,4) == "http") {
				...
				$image = HTTPS_IMAGE . 'no_image.png';
			}
			...
			$product_frees = $this->model_catalog_product->getProductFree($result['product_id']);

			foreach ($product_frees  as $product_free) {
				if (($product_free['date_start'] == '0000-00-00' || strtotime($product_free['date_start']) < time()) && ($product_free['date_end'] == '0000-00-00' || strtotime($product_free['date_end']) > time())) {
					$free = $this->currency->format($product_free['dollarvalue'], $this->config->get('config_currency'));
	...
	protected function getForm() {
		...
		if (isset($this->request->post['product_free'])) {
			$product_free = $this->request->post['product_free'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_free = $this->model_catalog_product->getProductFree($this->request->get['product_id']);
		} else {
			$product_free = array();
		}
		
		$data['product_frees'] = array();

		foreach ($product_free as $productfree) {
			$data['product_frees'][] = array(
				'store_id' => $productfree['store_id'],
				'quantity' => $productfree['quantity'],
				'dollarvalue' => $productfree['dollarvalue'],
				'date_start' => ($productfree['date_start'] != '0000-00-00') ? $productfree['date_start'] : '',
				'date_end' => ($productfree['date_end'] != '0000-00-00') ? $productfree['date_end'] :  ''
			);
		}
		// Image
		...
		} elseif (!empty($product_info) && substr($product_image['image'],0,4) == "http") {
			$data['thumb'] =  $product_info['image'];
		} else {
			$data['thumb'] = HTTPS_IMAGE . 'no_image.png';
		}
		...
		foreach ($product_images as $product_image) {
			if (substr($product_image['image'],0,4) == "http") {
				...
				$image = HTTPS_IMAGE . 'no_image.png';
                $thumb = HTTPS_IMAGE . 'no_image.png';
			}
