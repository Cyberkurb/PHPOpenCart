...
class Deactivate {
    ...
    public function __construct($data) {
        ...
        $this->app_slug = 'youtube-gallery';
        $this->app_name = 'Youtube Gallery';
    ...
    public function setDeactivateReasons() {
        return require ( (__DIR__) . '/youtube-gallery-assets/reasons.php' );
    }
...
class ControllerExtensionModuleElfsightYoutubeGallery extends Controller {
    private $error = array();

    public $params = NULL;

    const ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/yottie/?utm_source=portals&utm_medium=opencart&utm_campaign=youtube-gallery&utm_content=sign-up';

    public function index() {
        $this->load->language('extension/module/elfsight_youtube_gallery');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('module_elfsight_youtube_gallery', $this->request->post);
        ...
        $data['action'] = $this->url->link('extension/module/elfsight_youtube_gallery', 'user_token=' . $this->session->data['user_token'], true);

        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

        if (isset($this->request->post['module_youtube_gallery_status'])) {
            $data['module_youtube_gallery_status'] = $this->request->post['module_youtube_gallery_status'];
        } else {
            $data['module_youtube_gallery_status'] = $this->config->get('module_youtube_gallery_status');
        }

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');

        $this->response->setOutput($this->load->view('extension/module/elfsight_youtube_gallery', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/elfsight_youtube_gallery')) {
    ...
    public function setDeactivateReasons() {
        return require ( './controller/extension/module/youtube-gallery-assets/reasons.php' );
    }
    ...
    public function uninstall() {
        ...
        include './controller/extension/module/youtube-gallery-assets/popup.php';
    }
