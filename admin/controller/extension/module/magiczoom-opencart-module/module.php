...
if (defined('HTTP_ADMIN')) {
    define ('MTOOLBOX_ADMIN_FOLDER_magiczoom',str_replace('catalog',preg_replace('/.*?([^\/]*)\/$/is','$1',HTTP_ADMIN),DIR_APPLICATION) . 'controller/'.$modulesPath.'/magiczoom-opencart-module/');
} else {
    define ('MTOOLBOX_ADMIN_FOLDER_magiczoom',DIR_APPLICATION . 'controller/'.$modulesPath.'/magiczoom-opencart-module/');
}

function magiczoom_LoadScroll($tool) {
    static $scroll = null;
    if($scroll === null) {
        $scroll = false;
        if($tool->params->checkValue('magicscroll', 'yes', 'product')) {
            require_once (MTOOLBOX_ADMIN_FOLDER_magiczoom.'magicscroll.module.core.class.php');
...
function magiczoom($content, $currentController = false , $type = false, $info = false) {
    ...
    $settings = $currentController->config->get('magiczoom_settings');
    $magiczoom_status = $settings['magiczoom_status'];
  

    if ($magiczoom_status != 0) {
	...
	if ($tool->params->profileExists($type)) { //for main profiles
		...
		return $content;
            ...
            $content = magiczoom_parse_contents($content,$currentController);
            //if ($oldContent != $content) $content = magiczoom_set_headers($content);
            $content = magiczoom_set_headers($content); //TODO ?
	    ...
	    if ($type == 'product' && $tool->type == 'standard') {
                ...
                $tool->params->setProfile($type);
                require_once (MTOOLBOX_ADMIN_FOLDER_magiczoom.'magictoolbox.templatehelper.class.php');
                MagicToolboxTemplateHelperClass::setPath(MTOOLBOX_ADMIN_FOLDER_magiczoom.'templates');
                MagicToolboxTemplateHelperClass::setOptions($tool->params);
                $scroll = magiczoom_LoadScroll($tool);
                $html = MagicToolboxTemplateHelperClass::render(array(
                    'main' => magiczoom_getMainTemplate($GLOBALS['magictoolbox']['prods_info'],$type),
                    'thumbs' => magiczoom_getSelectorsTemplate($GLOBALS['magictoolbox']['prods_info'],$type),
		...
		foreach ($GLOBALS['magictoolbox']['prods_info'] as $product) {
		    ...
		    $img = magiczoom_getThumb($src,'original',$product['product_id']);
		    $thumb = magiczoom_getThumb($src,$position.'-thumb',$product['product_id']);
		    $thumb2x = magiczoom_getThumb($src,$position.'-thumb2x',$product['product_id']);                
		...
		$GLOBALS['magictoolbox']['plinks'] = array();
                ...
                if (isset($GLOBALS['magictoolbox']['prods_info']['gallery_images']) && count($GLOBALS['magictoolbox']['prods_info']['gallery_images'])) {
                    foreach ($GLOBALS['magictoolbox']['prods_info']['gallery_images'] as $image) {
                        ...
                        $img = magiczoom_getThumb($src,'original',$pid);
                        $medium = magiczoom_getThumb($src,'thumb',$pid);
...
function magiczoom_set_headers ($content, $headers = false, $controller = false) {


    if (isset($GLOBALS["magictoolbox"]["magiczoom"])) {
        $plugin = $GLOBALS["magictoolbox"]["magiczoom"];
    } else {
        $plugin = magiczoom_load_core_class($controller);  
    } 
    ...
    $scroll = magiczoom_LoadScroll($plugin);
    if($scroll && isset($GLOBALS['magictoolbox']['page_type']) && $GLOBALS['magictoolbox']['page_type'] == 'product') {
    ...
    if (!count($matches)) { //check if already present
        ...
        $headers .= '<script type="text/javascript">
			...
			if(typeof(magicJS.Doc.je1) == "undefined") magicAddEvent = "jAddEvent";
					    ...
					    MagicZoom.update(\'MagicZoomImage\'+ zoomId, MagicZoomOptiMages[optId][id][\'original\'], MagicZoomOptiMages[optId][id][\'thumb\']);
                                        }
        ...
        //if (!empty($GLOBALS['magictoolbox']['page_type']) && $GLOBALS['magictoolbox']['page_type'] == 'product') {
            ...
            <script type=\"text/javascript\" src=\"".$prefix."catalog/view/javascript/magiczoom_product.js\"></script>
            ";
        //}
...
function &magiczoom_load_core_class($currentController = false) {
    ...
    if(!isset($GLOBALS["magictoolbox"]["magiczoom"])) {
        /* load core class */
        require_once (MTOOLBOX_ADMIN_FOLDER_magiczoom.'magiczoom.module.core.class.php');
	...
	foreach($settings as $param_name => $param_value) {
		...
		if (!preg_match('/watermark/is',$param_name)) {
		    $GLOBALS["magictoolbox"]["magiczoom"]->params->setValue($matches[2],$param_value,$matches[1]);
		} else if (preg_match('/default_watermark/is',$param_name)) {
		    $GLOBALS["magictoolbox"]["magiczoom"]->params->setValue($matches[2],$param_value,$matches[1]);
		    $GLOBALS["magictoolbox"]["magiczoom"]->params->setValue(str_replace('default_','',$param_name),$param_value,'default');
		}
	    }
	} 

        $GLOBALS['magictoolbox']['product-zoomMode'] = $GLOBALS["magictoolbox"]["magiczoom"]->params->getValue('zoomMode');

    }
    return $GLOBALS["magictoolbox"]["magiczoom"];
}

function magiczoom_parse_contents($content,$currentController) {
    ...
    if ($type == 'product') { //use only on product page
	  ...
	  if (is_array($opt) && count($opt)) {
		  ...
		  if (!empty($option_value)) {
		      $jsOptions[$opt_id][$option['product_option_value_id']]['original'] = magiczoom_getThumb('image/'.$option_value,'original');
		      $jsOptions[$opt_id][$option['product_option_value_id']]['thumb'] = magiczoom_getThumb('image/'.$option_value,'thumb');
		  }
	      }
	  }
      }
    ...
    if ($type == 'product') {
	    ...
	    $content = magiczoom_category_like_replace($products,$type,$content);
	} else {
	    $content = magiczoom_cut_boxes($products,$type,$content); //cut all product images
	}

    } 

    if (isset($jsOptions) && count($jsOptions)  && $content != $oldContent) $content = str_replace('</head>','<script type="text/javascript"> MagicZoomOptiMages = '.json_encode($jsOptions).'; </script></head>',$content);

    return $content;
}

function magiczoom_getProductParams ($id, $params = false) {
...
function magiczoom_getThumb($src, $size = null, $pid = null) {
    if($size === null) $size = 'thumb';
    require_once (MTOOLBOX_ADMIN_FOLDER_magiczoom.'magictoolbox.imagehelper.class.php');
    ...
    if (!isset($GLOBAL['imagehelper'])) {
        $GLOBAL['imagehelper'] = new MagicToolboxImageHelperClass($shop_dir, '/'.$image_dir.'magictoolbox_cache', $GLOBALS["magictoolbox"]["magiczoom"]->params, null, $url);
    }
    return preg_replace('/https?\:\/\//is','//',$GLOBAL['imagehelper']->create('/' . $src, $size, $pid));
}

function magiczoom_getMainTemplate($info,$type) {
    ...
    $img = magiczoom_getThumb($src,'original',$id);
    $thumb = magiczoom_getThumb($src,null,$id);
    $thumb2x = magiczoom_getThumb($src,'thumb2x',$id);                
...
function magiczoom_getSelectorsTemplate($info,$type) {
	...
	foreach ($images as $image) {
	    ...
	    $img = magiczoom_getThumb($src,'original',$id);
	    $medium = magiczoom_getThumb($src,null,$id);
	    $medium2x = magiczoom_getThumb($src,'thumb2x',$id);
	    $thumb = magiczoom_getThumb($src,'selector',$id);
	    $thumb2x = magiczoom_getThumb($src,'selector2x',$id);                
...
function magiczoom_set_params_from_config ($config = false) {
    if ($config) {
        $plugin = $GLOBALS["magictoolbox"]["magiczoom"];
...
function magiczoom_use_effect_on(&$tool) {
...
function magiczoom_category_like_replace ($products, $type, $content) {
	...
	if (isset($matches[1])) {
        ...
        $readyImage = str_replace('$','\$',magiczoom_getMainTemplate($product,$type)); //backslash the dollar char in path
...
function magiczoom_cut_boxes ($products,$type,$content) {
