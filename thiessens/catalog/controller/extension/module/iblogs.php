...
class ControllerExtensionModuleiBlogs extends Controller
{
    ...
    protected function category($data)
    {
        ...
        $data['no_image']         = $this->model_tool_image->resize('no_image.png', $blog_listing['image_width'], $blog_listing['image_height']);
        $data['category_thumb']   = $category ? $this->model_tool_image->resize($category['image'], $blog_listing['image_width'], $blog_listing['image_height']) : $data['no_image'];
        ...
        foreach ($posts as $key => $post) {
            $data['posts'][$key] = $post;
            $data['posts'][$key]['image'] = $post['image'] ? $this->model_tool_image->resize($post['image'], $blog_listing['image_width'], $blog_listing['image_height']) : $data['no_image'];
    ...
    protected function post($data)
    {
        ...
        $data['no_image']         = $this->model_tool_image->resize('no_image.png', $post_view['image_width'], $post_view['image_height']);

        $data['show_main_image'] = $post_view['main_image'] && $post['image'];
        $data['main_image'] = '';
        if ($post['image']) {
            $data['main_image'] = $this->model_tool_image->resize($post['image'], $post_view['image_width'], $post_view['image_height']);
        }
        ...
        if ($related_post) {
            ...
            foreach ($related_post as $key => $rel_post) {
                ...
                $data['related_post'][$key]['image'] = $rel_post['image'] ? $this->model_tool_image->resize($rel_post['image'], $blog_listing['image_width'], $blog_listing['image_height']) : $data['no_image'];
