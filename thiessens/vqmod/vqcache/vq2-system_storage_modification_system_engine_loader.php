	...
	protected function callback($registry, $route, $model_instance=null) {
		return function($args) use($registry, $route, $model_instance) {
			...
			if ($result && !$result instanceof Exception) {
				...
				$class = ($model_instance) ? get_class($model_instance) : 'Model' . preg_replace('/[^a-zA-Z0-9]/', '', substr($route, 0, strrpos($route, '/')));
				
				// Store the model object
				$key = substr($route, 0, strrpos($route, '/'));
				
				if (!isset($model[$key])) {
					$model[$key] = ($model_instance) ? $model_instance : new $class($registry);
				}
