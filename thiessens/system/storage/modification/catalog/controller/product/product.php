	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['image']) {
				...
				// Product Image 360
				$data['image360'] = $product_info['image360'];
				// END Product Image 360
			
	
				// FANCYBOX
				if ($product_info['image']) {
					$data['full_image'] = "/image/" . $product_info['image'];
				} else {
					$data['full_image'] = '';
				}
				// END OF FANCYBOX
			...
			$data['options'] = array();
				...
				$data['text_model'] = $this->config->get('module_eb_color_size_option_model_title'.$this->config->get('config_language_id'));
				$data['text_sku_c'] = $this->config->get('module_eb_color_size_option_sku_title'.$this->config->get('config_language_id'));
				$data['text_upc_c'] = $this->config->get('module_eb_color_size_option_upc_title'.$this->config->get('config_language_id'));
				$data['text_ean_c'] = $this->config->get('module_eb_color_size_option_ean_title'.$this->config->get('config_language_id'));
				$data['text_jan_c'] = $this->config->get('module_eb_color_size_option_jan_title'.$this->config->get('config_language_id'));
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				'colorimage'               => $this->model_tool_image->resize($option_value['image'], $data['module_eb_color_size_option_bullet_width'], $data['module_eb_color_size_option_bullet_height']),
			...
			$data['attribute_groups'] = $this->model_catalog_product->getProductAttributes($this->request->get['product_id']);
				...
				$data['sizing_guides'] = $this->model_extension_rciextension_sizing->getProductSizings($this->request->get['product_id']);
			
	
				$this->load->language('extension/rciextension/technology');
				$this->load->model('extension/rciextension/technology');
				$data['technologies'] = $this->model_extension_rciextension_technology->getProductTechnologies($this->request->get['product_id']);
			

				$this->load->model('account/wishlist');
				if ($this->customer->isLogged()) {
					$data["wishlist_items"] = array_column($this->model_account_wishlist->getWishlist(),'product_id');
				} else {
					$data["wishlist_items"] = isset($this->session->data['wishlist']) ? $this->session->data['wishlist'] : array();
				}
	...
	public function write() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ((utf8_strlen($this->request->post['name']) < 3) || (utf8_strlen($this->request->post['name']) > 25)) {
				...
				if ((utf8_strlen($this->request->post['subject']) < 5) || (utf8_strlen($this->request->post['subject']) > 255)) {
