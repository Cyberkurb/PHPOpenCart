...
class ModelCheckoutOrder extends Model {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				foreach ($product['option'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', 
				`value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "',`ebproduct_option_combination_id` = '" . (isset($option['ebproduct_option_combination_id']) ? (int)$option['ebproduct_option_combination_id'] : 0) . "'
			");
				}
			}
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				if($order_option['ebproduct_option_combination_id']){
					$ebquery = $this->db->query("SELECT * FROM ".DB_PREFIX."ebproduct_option_combination WHERE ebproduct_option_combination_id = '".(int)$order_option['ebproduct_option_combination_id']."'");
					if($ebquery->row){
						$ebquery2 = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value WHERE product_option_value_id = '" . (int)$ebquery->row['product_option_value_id'] . "' AND subtract = '1'");
						if($ebquery->row){
							$this->db->query("UPDATE " . DB_PREFIX . "ebproduct_option_combination SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE ebproduct_option_combination_id = '" . (int)$ebquery->row['ebproduct_option_combination_id'] . "'");
						}
					}
				}
			...
			if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				if($order_option['ebproduct_option_combination_id']){
					...
					if($ebquery->row){
						$ebquery2 = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value WHERE product_option_value_id = '" . (int)$ebquery->row['product_option_value_id'] . "' AND subtract = '1'");
						if($ebquery->row){
							$this->db->query("UPDATE " . DB_PREFIX . "ebproduct_option_combination SET quantity = (quantity + " . (int)$product['quantity'] . ") WHERE ebproduct_option_combination_id = '" . (int)$ebquery->row['ebproduct_option_combination_id'] . "'");
						}
					}
				}
    ...
    public function cartValue(){
		$sql_1 = "SELECT sum(quantity) AS items_count FROM " . DB_PREFIX . "cart WHERE session_id = '" . $this->db->escape($this->session->getId()) . "';";
		$query = $this->db->query($sql_1);
		return $query->row['items_count'];
	}
