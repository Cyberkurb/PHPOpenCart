...
$this->load->model('tool/image');
		...
		if (isset($this->request->post['product_option'])) {
			...
			$product_options = array();
				...
				$data['optioncolors'] = array();
				if(isset($this->request->get['product_id'])){
					$data['optioncolors'] = $this->model_catalog_product->getebProductOptionColor($this->request->get['product_id']);
				}
		...
		foreach ($product_specials as $product_special) {
			$data['product_specials'][] = array(
				...
				// Product Image 360
				if (isset($this->request->post['image360'])) {
					$data['image360'] = $this->request->post['image360'];
				} elseif (!empty($product_info)) {
		...
		foreach ($product_images as $product_image) {
			if (is_file(DIR_IMAGE . $product_image['image'])) {
				...
				if(isset($product_image['product_image_id'])){
					$productoptionvalueImages = $this->model_catalog_product->getebProductoptionvalueImages($product_image['product_image_id']);
				}else{
				  $productoptionvalueImages = array(); 	
				}
			
			$data['product_images'][] = array(
'productoptionvalueImages'	=> $productoptionvalueImages,
				'image'      => $image,
				...
				if (isset($this->request->post['sizing_guides'])) {
					$sizingGuides = $this->request->post['sizing_guides'];
				} elseif (isset($this->request->get['product_id'])) {
					$sizingGuides = $this->model_extension_rciextension_sizing->getProductSizingsLinks($this->request->get['product_id']);
				} else {
					$sizingGuides = array();
				}

				$data['sizing_guides'] = array();

				foreach ($sizingGuides as $sizingGuideId) { 
					
					$sizingGuide = $this->model_extension_rciextension_sizing->getSizing($sizingGuideId);

					if ($sizingGuide) {
						$data['sizing_guides'][] = array(
							'sizing_id'   => $sizingGuide['sizing_id'],
				...
				if (isset($this->request->post['technologies'])) {
					$technologies = $this->request->post['technologies'];
				} elseif (isset($this->request->get['product_id'])) {
					$technologies = $this->model_extension_rciextension_technology->getProductTechnologiesLinks($this->request->get['product_id']);
				} else {
					$technologies = array();
				}
				...
				foreach ($technologies as $technologyId) { 
					...
					$technology = $this->model_extension_rciextension_technology->getTechnology($technologyId);
