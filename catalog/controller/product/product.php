...
class ControllerProductProduct extends Controller {
	...
	public function index() {
        ...
        elseif (isset($this->request->get['product'])){
            $product_slug = $this->request->get['product'];
        }
        elseif (isset($this->request->get['article'])){
            $recipe_old = $this->request->get['article'];
        }
        else {
			$product_id = 0;
		}
        ...
        elseif (isset($this->request->get['product'])){
            $product_info = $this->model_catalog_product->getOldProduct($product_slug);  
            $this->request->get['product_id'] = $product_info['product_id'];
        }
        elseif (isset($this->request->get['article'])){
            $product_info = $this->model_catalog_product->getOldRecipe($recipe_old);
            $this->request->get['product_id'] = $product_info['product_id'];
            //echo var_dump($product_info);
            //exit;
        }
        else{
            $product_info = $this->model_catalog_product->getProduct($product_id);
		...
		if ($product_info) {
          ...
          if($categories) {
            ...
            $product_documents = $this->model_catalog_product->downloadsProduct($this->request->get['product_id']);
			...
			$data['keywords'] = $product_info['meta_keyword'];
			//$this->document->addLink($this->url->link('product/product', 'product_id=' . $this->request->get['product_id']), 'canonical');
			$this->document->addScript('https://images.pitboss-grills.com/javascript/jquery/magnific/jquery.magnific-popup.min.js');
			$this->document->addStyle('https://images.pitboss-grills.com/javascript/jquery/magnific/magnific-popup.css');
			...
			$product_exclusive = $this->model_catalog_product->exclusiveProduct($this->request->get['product_id']);

			$data['reviews_seo'] = array();
				$page = 1;
			$reviews_for_seo = $this->model_catalog_review->getReviewsByProductId($this->request->get['product_id'], ($page - 1) * 5, 5);

			foreach ($reviews_for_seo as $result) {
				$data['reviews_seo'][] = array(
			...
			if($product_exclusive['name']){
				$data['exclusive'] = $product_exclusive['name'];
				$data['exclusive_url'] = $product_exclusive['text'];
			}
			else{
			...
			$data['preptime'] = $this->model_catalog_product->getPrepTime($this->request->get['product_id']);
			$data['cooktime'] = $this->model_catalog_product->getCookTime($this->request->get['product_id']);
			$data['difficulty'] = $this->model_catalog_product->getDifficulty($this->request->get['product_id']);
			$data['servings'] = $this->model_catalog_product->getServings($this->request->get['product_id']);
			...
			if($this->config->get('config_store_id') == 10){
				$data['quantity'] = $product_info['quantity_ca'];	
			}
			...
			if ((float)$product_info['special']) {
				...
				if($discounted_amount > 0){
					$data['s_savings'] = $this->currency->format($this->tax->calculate($discounted_amount, $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
				}
				else{
					$data['s_savings'] = false;
				}
			} else {
				$data['s_savings'] = false;
			}
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[] = array(
        					...
        					'stock_status_id'      => $option_value['stock_status_id'],
			...
			foreach ($results as $result) {
                ...
                $cats_info = $this->model_catalog_product->getCategories($result['product_id']);
                ...
                if($parent_name == 'Recipes'){
                    ...
                    $data['recipes'][] = array(
                        ...
                        'thumb'       => $image,
                        'name'        => $result['name'],
                        'description' => utf8_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 250) . '..',
                        'price'       => $price,
                        'special'     => $special,
                        'tax'         => $tax,
			...
			if($product_info['isbn']== 0){
				$this->response->setOutput($this->load->view('product/product', $data));
			}
			elseif($product_info['isbn']== 1){
				$this->response->setOutput($this->load->view('product/recipe', $data));
			}
			elseif($product_info['isbn']== 2){
				$this->response->setOutput($this->load->view('product/blog', $data));
			}
			elseif($product_info['isbn']== 3){
				$this->response->setOutput($this->load->view('product/blog_full', $data));
			}
			elseif($product_info['isbn']== 4){
				$this->response->setOutput($this->load->view('product/blog_full_nh', $data));
			}
		} else {
			$url = '';
	...
	public function review_seo() {
		...
		$this->response->setOutput($this->load->view('product/review_seo', $data));
	}
