	...
	public function send() {
        ...
        $transaction = array(
            ...
            "Document"  => array("Head"  =>  array(
                    array("Name" => "InvoiceNumber", "Value" => "DWS".$order_info['order_id']),
                    array("Name" => "PONumber", "Value" => "DWS".$order_info['order_id'])
            )),
            "SetupId"   => $setupid,
            "Tender"    => "CreditCard",
            "Shipto"    => array(
        ...
        if($transaction_status == 'Approved'){
            ...
            if($this->config->get('config_store_id') == 14){
                ...
                $this->model_extension_payment_payfabric->userValidation($username, $casenumber, $this->session->data['order_id']);
            }
            ...
            $this->session->data['error'] = "We have high security when processing credit cards for your protection. Your credit card didn't complete its process, please check your payment information to ensure it matches the address on the credit card and try again.";
