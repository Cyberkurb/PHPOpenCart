...
class ControllerEventActivity extends Controller {
    ...
    // model/account/product/addProduct/after
	public function addProduct(&$route, &$args, &$output) {
		if ($this->config->get('config_custprod_activity')) {
			$this->load->model('account/activity');
			if($this->config->get('config_store_id') == 14){
				$username = $this->session->data['order_entry_user'];
			}
			...
			$this->model_account_activity->addActivity('regprod', $activity_data);
		}
	}

	// model/account/sponsor/addSponsor/after
	public function addSponsor(&$route, &$args, &$output) {
		if ($this->config->get('config_custprod_activity')) {
			$this->load->model('account/activity');
