...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "order SET username = '" . $this->db->escape($this->session->data['order_entry_user']) . "' WHERE order_id = '" . (int)$order_id . "';");
		
		$ch = curl_init("https://crmorder.pitboss-grills.com/order_attemptv2.php?order_id=".$order_id);
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			curl_close($ch);
			/*
			if((int)$order_info['store_id'] != 15){
				if((int)$order_status_id != 0){
					$ch = curl_init("https://crmorder.pitboss-grills.com/activecamp_sale.php?customer_id=".$order_info['customer_id'] . "&order_id=" . (int)$order_id);
					curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
					curl_exec($ch);
					curl_close($ch);
				}
	...
	public function zipcodeproblem_check($zipcode){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "problem_zipcodes WHERE zipcode = '" . $this->db->escape($zipcode) . "';");
